...
function DrawUtil() {
  ...
  this.drawProgressBar = function (stats) {
    ...
    return this.drawThisMany(passWidth, 'o', colorPass) +
           this.drawThisMany(failWidth, 'X', colorFail) +
           this.drawThisMany(skipWidth, '-', colorSkip);
  };

  // draw `n` many of `char` in `color`
  ...
  this.drawChart = function (stats) {
    write(colorGraphOutline(topLine) + '\n');
    ...
    write(colorGraphOutline(bottomLine) + '\n');
    this.cursorUp(3);
  };
