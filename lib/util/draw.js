...
var frameCounter = 0;
// unicode braille characters for animation
var progressAnimation = ['\u2807', '\u2826', '\u2834', '\u2838', '\u2819', '\u280b',
                         '\u280f', '\u2827', '\u2836', '\u283c', '\u2839', '\u281b' ];
// alternative to use -- emoji clock
var progressAnimation2 = ['\u{1F55B}', '\u{1F550}', '\u{1F551}', '\u{1F552}', '\u{1F553}', '\u{1F554}',
...
function DrawUtil() {
  ...
  this.drawThisMany = function(n, char, color, bg) {
    var outputString = '';
    if (bg === 'green') {
      var colorBrowser = clc.bgGreen.xterm(color);
    } else if (bg === 'red') {
      var colorBrowser = clc.bgRed.xterm(color);
    } else if (bg === 'yellow') {
      var colorBrowser = clc.bgYellow.xterm(color);
    }
    for (var i = 0; i < n; i++) {
